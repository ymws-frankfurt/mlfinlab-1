...
class BaseBars(ABC):
    ...
    # o3-mini: 3. Parallel CSV Writing (Optional Enhancement)
    def batch_run(self, file_path_or_df: Union[str, Iterable[str], pd.DataFrame, dd.DataFrame], 
                  batch_size: int = 10**6, verbose: bool = True, 
                  to_csv: bool = False, output_path: Optional[str] = None) -> Union[pd.DataFrame, dd.DataFrame, None]:
        """
        Reads CSV file(s) or Pandas/Dask DataFrame in batches and constructs financial data structure.

        :param file_path_or_df: (str, iterable of str, Pandas DataFrame, or Dask DataFrame)
        :param batch_size: (int) Number of rows per batch.
        :param verbose: (bool) Prints batch processing messages.
        :param to_csv: (bool) If True, writes output to a CSV file.
        :param output_path: (Optional[str]) Path to save the output CSV.

        :return: (pd.DataFrame or dd.DataFrame or None) Processed bars.
        """
        
        if to_csv:
            header = True  # Write header only once
            open(output_path, 'w').close()  # Clear existing file
        ...
        for batch in self._batch_iterator(file_path_or_df, batch_size):
            if verbose:
                print(f'Processing batch {count}...')

            list_bars = self.run(data=batch)  # Process batch
            
            # Convert processed batch to Pandas DataFrame
            df_bars = pd.DataFrame(list_bars, columns=cols)

            if to_csv:
                df_bars.to_csv(output_path, mode='a', header=header, index=False)
                header = False  # Only write header for the first batch
            else:
                dask_batches.append(dd.from_pandas(df_bars, npartitions=1))
        ...
        # If not writing to CSV, return combined Dask DataFrame
        return dd.concat(dask_batches) if not to_csv else None


    # o3-mini
    def _batch_iterator(self, file_path_or_df: Union[str, Iterable[str], pd.DataFrame], batch_size: int = 10**6):
        """
        Optimized batch iterator that reads CSV files using Dask in chunks to prevent memory overflow.
        
        :param file_path_or_df: (str, iterable of str, or pd.DataFrame) Input CSV path(s) or DataFrame.
        :param batch_size: (int) Number of rows per batch.
        :yield: (pd.DataFrame) A batch of data as Pandas DataFrame.
        """

        # If input is a DataFrame, yield Pandas chunks
        if isinstance(file_path_or_df, pd.DataFrame):
            for i in range(0, len(file_path_or_df), batch_size):
                yield file_path_or_df.iloc[i : i + batch_size]

        # If input is a CSV path or list of CSVs, use Dask for efficient reading
        elif isinstance(file_path_or_df, (str, Iterable)):
            ddf = dd.read_csv(file_path_or_df, assume_missing=True, blocksize=batch_size * 64)
            
            # Yield Pandas DataFrame batches for processing
